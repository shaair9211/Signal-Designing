...
my_font = pygame.font.SysFont('abc', 30)
...
while shararti_molana < 300*fps:
    ...
    expectancy = 0.045
    weight = 1.5
    expectancy_i, expectancy_j, expectancy_k, expectancy_l = (expectancy*0.00921*4*weight), (expectancy*0.031*4*weight), (expectancy*0.00921*4*weight), (expectancy*0.0355*4*weight)
    len_li = 0
    len_ti = 0
    len_ri = 0
    len_lj = 0
    ...
    if t == True:
        m = fps * (((len_phase_i**1.5) * 0.01)+2)
        n = fps * (((len_phase_j**1.5) * 0.01)+2)
        o = fps * (((len_phase_k**1.5) * 0.01)+2)
        s = fps * (((len_phase_l**1.5) * 0.01)+2)
    ...
    if karan_johar >= 60:
        ...
        karan_johar == 0
        shararti_molana += 1
        average_a = round(sum(density_i) / len(density_i), 4)
        average_b = round(sum(density_j) / len(density_j), 4)
        average_c = round(sum(density_k) / len(density_k), 4)
        average_d = round(sum(density_l) / len(density_l), 4)
        print("The average of the list is:", average_a, average_b, average_c, average_d)
        speed = my_font.render('Speed: 4x', False, (255, 255, 255))
        volume = my_font.render(f'Volume:{eleven}', False, (255, 255, 255))
        text_di = my_font.render(f'Average Queue Length WB = {average_a}', False, (255, 255, 255))
        text_dj = my_font.render(f'Average Queue Length NB= {average_b}', False, (255, 255, 255))
        text_dk = my_font.render(f'Average Queue Length EB= {average_c}', False, (255, 255, 255))
        text_dl = my_font.render(f'Average Queue Length SB= {average_d}', False, (255, 255, 255))

        screen.blit(speed, (10, 10))
        screen.blit(volume, (10, 35))
        screen.blit(text_di, (10, 250))
        screen.blit(text_dj, (710, 10))
        screen.blit(text_dk, (840, 510))
        screen.blit(text_dl, (150, 750))
...
sheet1 = book.get_sheet_by_name(eleven) if eleven in book.sheetnames else book.create_sheet(eleven)
